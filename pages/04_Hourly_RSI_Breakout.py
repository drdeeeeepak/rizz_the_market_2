...
# ══════════════════════════════════════════════════════════════════════════════
# Backtest Divergence-Confirmed Breakout Trades
# ══════════════════════════════════════════════════════════════════════════════

st.divider()
with st.expander("⚡ Backtest: Divergence-Confirmed Breakout Trades (All Thresholds)", expanded=False):
    st.markdown("""
    **Test pure divergence-confirmed breakout trades across all OS/OB thresholds.**

    This backtest shows performance when taking ONLY trades that have:
    - HL/LH pivot breakout detected
    - **AND** divergence confirmation (price makes new high/low but RSI doesn't confirm it)

    No other filters, no additional rules — just divergence-confirmed breakouts.
    """)

    if df_1h is not None and not df_1h.empty:
        st.info("🔄 Running divergence-only threshold scan on 1h data...")

        try:
            import data.reversal_backtest as rb

            # Divergence-only threshold scan for 1h data
            div_scan = rb.threshold_scan(
                df_1h,
                timeframe_label="1h (Divergence Only)",
                rsi_period=14,
                ob_os_pairs=((40, 60), (45, 55), (50, 50), (55, 45)),
                ...
                div_lookback=20,
                div_min_gap=2.0
            ) if hasattr(rb, 'threshold_scan') else pd.DataFrame()
            ...
            if not div_scan.empty:
                ...
                with col1:
                    st.metric("Best OS/OB", f"{best_row['os']:.0f}/{best_row['ob']:.0f}")
                with col2:
                ...
                st.markdown("### Divergence Breakout Performance by Threshold:")
                st.dataframe(
                    div_scan_sorted[['os', 'ob', 'n_trades', 'win_rate', 'expectancy_pts',
                ...
                st.download_button(
                    ...
                    file_name="divergence_breakout_only_backtest.csv",
                    mime="text/csv",
                    key="p04_div_backtest_download"
                )
                ...
                - **Best threshold:** OS {best_row['os']:.0f}/OB {best_row['ob']:.0f} with {best_row['expectancy_pts']:.2f} pts/trade expectancy
                ...
                ✅ **Recommendation:** Use **OS {best_row['os']:.0f}/OB {best_row['ob']:.0f}** for divergence-only breakout trading
                (Best balance of trade frequency and expectancy)
                """)

            else:
                st.info("Divergence backtest module not available for 1h timeframe in this session.")

        except Exception as e:
            st.info(f"Divergence backtest not available: {str(e)}")
    else:
