# pages/02_Nifty_EMA_Ribbon.py — v2.1 (RTR & VIX-Pace Update)
...
# ── DTE & Threat Multiplier (RTR Proxy) ───────────────────────────────────────
try:
...
_thr_sub  = f"Ret {daily_ret_pct:+.1f}% · RelRng {rel_range:.2f}x"
_yday_thr_sub = (f"Yday: CE {_yday_ce_thr:.2f} · PE {_yday_pe_thr:.2f}"
                 if (_yday_ce_thr > 0 or _yday_pe_thr > 0) else "")

# Define basic DTB strings for the top metrics card (VIX Pace Fixed)
...
# ── Threat & DTB daily history table — RTR + VIX-Adjusted ─────────────
