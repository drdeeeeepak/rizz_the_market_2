...
# Define basic DTB strings for the top metrics card
...
with c6: ui.metric_card("DAYS TO BREACH", _dtb_val,
                         sub=_dtb_sub,
                         color="default")
# ── Threat & DTB daily history table — VIX-Adjusted Current Cycle ─────────────
with st.expander("Threat & DTB — Daily History (VIX-Adjusted)", expanded=False):
    try:
        from data.live_fetcher import get_nifty_daily_live as _gdl
        _dlive = _gdl() 
    except Exception:
    ...
    if tue_anchor_available and ce_def_trig_spot > 0 and pe_def_trig_spot > 0 and not _dlive.empty:
        ...
        _th["rel_vol"] = _th["volume"] / _th["volume"].rolling(14).mean()
        
        # 2. VIX-Adjusted Expected Move (Rule of 16)
        # Sets a 'Volatility Floor' so DTB remains realistic even on flat days
        ...
        # Gap is computed against the defensive trigger spots (2.5% adverse)
        # These variables update automatically if a roll anchor is detected
        ...
        # Find the start of the current biweekly trade cycle
        _days_since_wed = (_today_d.weekday() - 2) % 7
        _current_wed    = _today_d - _dt.timedelta(days=_days_since_wed)
        
        # Only show dates belonging to the current trade
        _cycle_mask = _th.index.date >= _current_wed
        _th_cycle   = _th[_cycle_mask].copy()
        
        _lc_date   = _dlive.index[-1].date() if hasattr(_dlive.index[-1], 'date') else _dlive.index[-1].to_pydatetime().date()
        _has_today = (_lc_date == _today_d)
        
        # Process historical days in the cycle (excluding live row)
        _hist_end = -1 if _has_today else len(_th_cycle)
        _th_hist  = _th_cycle.iloc[:_hist_end]
        _display_rows = []
        for _idx, _r in _th_hist.iterrows():
            # DTB uses max of actual move or VIX-implied move to prevent false security
            ...
            _display_rows.append({
                ...
                "CE DTB":    f"{_r['ce_gap'] / _move:.1f}d",
                "PE DTB":    f"{_r['pe_gap'] / _move:.1f}d",
            })
        # 5. Live Row with Real-Time Pace vs. VIX Floor
        if _has_today:
            ...
            _display_rows.append({
                ...
                "CE DTB":    f"{_ce_gap_pts / _ce_pace:.1f}d",
                "PE DTB":    f"{_pe_gap_pts / _pe_pace:.1f}d",
            })
        
        _tbl = pd.DataFrame(list(reversed(_display_rows)))
        # 6. Heatmap Styling for "ACT" (Red) and "PREPARE" (Amber) thresholds
        def _style_urgent(row):
            styles = [""] * len(row)
            try:
                for i, col in [(5, "CE DTB"), (6, "PE DTB")]:
                    if float(row[col][:-1]) < 2.0: # Breach imminent in < 2 days
                        styles[i] = "background-color:#7f1d1d;color:white;font-weight:700"
                if float(row["CE Threat"]) > 1.15: styles[3] = "color:#ef4444;font-weight:700"
                if float(row["PE Threat"]) > 1.15: styles[4] = "color:#ef4444;font-weight:700"
            except: pass
            return styles
        if not _tbl.empty:
        ...
        st.info("VIX-Adjusted history requires an active Expiry Anchor.")
if not tue_anchor_available:
