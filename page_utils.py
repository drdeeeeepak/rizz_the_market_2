# page_utils.py — premiumdecay shared page utilities
# Bootstrap signals + live spot + last-updated caption for every page.
# Import this instead of the raw session_state check on every page.
...
def _fmt_ist(dt: datetime.datetime) -> str:
    """Format a datetime to readable IST string."""
...
def bootstrap_signals() -> tuple[dict, float, str]:
    ...
    if not sig:
        try:
            ...
            if sig:
                sig_source = "json"
                st.session_state["signals"] = sig  # populate for other pages
        except Exception:
            sig = {}
    ...
    spot = get_nifty_spot()
    if spot == 0:
        # Fallback: reconstruct from signals
        spot = sig.get("final_put_short", 0) + sig.get("final_put_dist", 0)
    if spot == 0:
        spot = float(sig.get("spot", 0))

    # ── Signals timestamp ───────────────────────────────────────────────────
    # Priority: _saved_at in sig dict (set by Home), then signals.json mtime
    ...
    try:
        if sig_source in ("json", "session"):
            ...
            if _signals_path.exists():
                ...
                # signals.json at repo root (fallback path)
...
def show_page_header(spot: float, signals_ts: str, page_key: str = "") -> None:
    ...
    Call this right after st.title() / st.caption().

    Example output:
      ⏱  Spot: 24,312  ·  Signals: 29 Apr 3:31 PM IST
    """
